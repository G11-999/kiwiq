...
async def test_org_selection(linkedin_client: LinkedInClient) -> None:
    ...
    try:
        ...
        # Test get_post_social_actions for a single organization post
        print("\nTesting get_post_social_actions for single organization post...")
       
        if org_posts:
            for test_post in org_posts[:10]:
                try:
                    # Get first organization post ID for testing
                    test_post_id = test_post.id
                    # test_post = org_posts[0]
                    
                    # Fetch social actions for single organization post
                    ########################################################################
                    ######################### GET SOCIAL ACTIONS FOR SINGLE ORG POST #########################
                    print("\n" + "="*80)
                    print("CALLING: get_post_social_actions()")
                    print(f"ARGS: {json.dumps([test_post_id])}")
                    print("KWARGS: {}")
                    print("="*80)
                    # linkedin_client.enable_caching = False
                    social_actions = await linkedin_client.get_post_social_actions(test_post_id)
                    
                    # Extract and display social action metrics
                    post_content = test_post.content_text[:50] + "..." if len(test_post.content_text) > 50 else test_post.content_text
                    likes = social_actions.likes_summary.total_likes if social_actions.likes_summary else 0
                    comments = social_actions.comments_summary.total_first_level_comments if social_actions.comments_summary else 0
                    
                    print(f"\nSocial actions for organization post:")
                    print(f"  Post: {post_content}")
                    print(f"    ID: {test_post_id}")
                    print(f"    Likes: {likes}")
                    print(f"    Comments: {comments}")
                    print()
                except Exception as e:
                    print(f"Error testing get_post_social_actions for organization post: {str(e)}")
                    import traceback
                    traceback.print_exc()
        else:
            print("No organization posts available to test get_post_social_actions.")

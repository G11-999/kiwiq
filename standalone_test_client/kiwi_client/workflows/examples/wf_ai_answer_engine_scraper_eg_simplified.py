...
workflow_graph_schema = {
    "nodes": {
        ...
        "ai_scraper": {
            ...
            "node_config": {
                # Return nested per-entity results in addition to flat query_results
...
async def validate_ai_scraper_output(
    ...
    # Check for all expected output fields (entity_results may be absent when flat mode)
    required_fields = [
        ...
        'query_results', 'executed_queries', 'used_cached_results'
    ]
    ...
    # Check entity results (present only when return_nested_entity_results=True)
    entity_results = outputs.get('entity_results') or {}
    if entity_results:
...
async def main_test_ai_scraper(
    ...
    if final_run_outputs:
        ...
        # Show entity results (only when nested output enabled)
        entity_results = final_run_outputs.get('entity_results') or {}
        if entity_results:
        ...
        # Always available in both flat and nested modes
        query_results = final_run_outputs.get('query_results', [])
        if query_results:
            print(f"\n📄 --- Sample Query Results ({len(query_results)} total) ---")
            for i, result in enumerate(query_results[:7]):
                print(f"\nResult {i+1}:")
                print(f"  Query: {result.get('query', 'N/A')}")
                print(f"  Provider: {result.get('provider', 'N/A')}")
                # Flat items include markdown and links; detailed timing/attempts live in MongoDB full records
                markdown = result.get('markdown')
                if markdown:
                    print(f"  Markdown: {str(markdown)[:200]}...")
                links = result.get('links') or []
                if links:
                    print(f"  Links: {min(len(links), 3)} shown")

"""
Testing Inputs Editor: edit workflow testing input variables for the selected workflow.
"""
...
st.set_page_config(page_title="Testing Inputs Editor", page_icon="🧰")
...
def main() -> None:
    st.title("Testing Inputs Editor")
    ...
    if mode == "Variable-by-variable":
        vars_dict = list_workflow_variables(workflow, "testing_inputs")
        if not vars_dict:
            st.info("No testing input variables found.")
            return

        var_name = st.selectbox("Variable (testing_inputs)", sorted(vars_dict.keys()), key="sel_test_inputs")
        ...
        if is_primitive:
            ...
            new_val_str = st.text_area("New Value", value=display_value, height=height, key=f"txt_test_inputs_{var_name}")
        else:
            editor_key = f"code_test_inputs_{var_name}"
            edited = code_editor(
        ...
        if st.button("Update", key=f"btn_test_inputs_{var_name}"):
            if is_primitive:
                ...
                try:
                    ...
                    st.error(f"Invalid value: {e}")
                    return
            ok = update_workflow_variable(workflow, "testing_inputs", var_name, new_value)
            if ok:
        ...
        try:
            ...
            file_path: Path = workflows_root / category / workflow_name / "wf_testing" / "wf_inputs.py"
        ...
        content_key = f"testing_full_content__{category}__{workflow_name}"
        ...
        use_syntax_highlight = st.checkbox(
            ...
            value=True, 
            key=f"testing_syntax_highlight__{category}__{workflow_name}"
        )
        ...
        if use_syntax_highlight:
            ...
            response = code_editor(
                ...
                height=600,
                response_mode="blur",
                key=f"testing_code_editor__{category}__{workflow_name}",
                allow_reset=True,
                buttons=custom_btns,
            )
            ...
            new_text = st.text_area(
                "Testing Inputs (full file)",
                value=st.session_state[content_key],
                height=height,
                key=f"testing_text_area__{category}__{workflow_name}",
            )
        ...
        # Save button
        if st.button("Save testing inputs file", type="primary"):
            try:
                ...
                st.success("Testing inputs file saved successfully!")
                st.rerun()
            except Exception as e:
